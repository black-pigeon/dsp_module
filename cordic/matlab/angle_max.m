...
for i=1:N
    ...
    fprintf(fid, 'tan(theta)is %f, angle(%i) is %f, cos_theta is %f, angle_sum is %f, factor is %f\n',1/(bitshift(1,i-1)), i, angle(i), cos(angle(i)), angel_sum, multi_factor);
    fprintf(fid_angle, '%x\n', fix_angle(i));
end
fclose(fid);
fclose(fid_angle);
fprintf('multi_factor is %f, angel_sum is %f, xn=%f\n',multi_factor, angel_sum, xn);